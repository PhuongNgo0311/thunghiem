...
export default function HomeScreen() {
  ...
  const fetchCategoryVideos = (category) => {
    fetch(
      `https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=10&type=video&q=${category}&key=AIzaSyBtMj8Yv_TZZ5aVGveZfgr3ONgu1qrcrrI`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data.items) {
  ...
  const fetchSuggestedVideos = () => {
    fetch(
      `https://youtube.googleapis.com/youtube/v3/videos?part=snippet&chart=mostPopular&maxResults=10&key=AIzaSyBtMj8Yv_TZZ5aVGveZfgr3ONgu1qrcrrI`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data.items) {
